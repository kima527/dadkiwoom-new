...
def run_trading_bot():
    ...
    while True:
        ...
        for rank, stock_data in enumerate(stock_results, 1):
            ...
            # ── ② 10:00~15:20 5볼린저 하한선 재매수 ──────────────
            # "5볼린저 하한선 돌파시" 재매수
            elif is_rebuy_window:
                if latest.get("signal_buy_bb_rebound") and sent_alerts[code].get("buy_reason") == "dynamic":
                    if sent_alerts[code]["buy_ema40"] != candle_time:
                        sent_alerts[code]["buy_ema40"] = candle_time

                        if not is_held:
                            qty = sent_alerts[code].get("sold_qty", 0)
                            if qty > 0:
                                order_res = client.place_buy_order(code, qty, price=close_price, order_type="3")
                                if order_res and order_res.get("return_code") == 0:
                                    msg = (
                                        f"🔄 <b>[재매수 - 5볼린저 하한선 돌파!]</b>\n"
                                        f"📊 우선순위: #{rank} (이격도: {disp_str})\n"
                                        f"종목: {name} ({code})\n"
                                        f"매수단가: {close_price:,.0f}원\n"
                                        f"매수수량: {qty}주\n"
                                    ...
                                    _add_alert("buy", f"재매수 [5볼린저하한선] {qty}주 @ {close_price:,.0f}원", code, name)
                                    sent_alerts[code]["sold_qty"] = 0  # Reset after rebuy
                                else:
                                    err_msg = order_res.get("return_msg") if order_res else "응답 없음"
                                    msg = (
                                        f"❌ <b>[재매수 실패 - 5볼린저 하한선]</b>\n"
                                        f"종목: {name} ({code})\n"
                                        f"에러내용: {err_msg}"
                                    )
                                    _add_alert("error", f"재매수실패: {err_msg}", code, name)
                                notifier.send_all(msg)
                            else:
                                logger.info(f"No previous sold quantity found for {name} ({code}) to rebuy. Skipping rebuy.")
                        else:
                            logger.info(f"Skipping rebuy (already held): {name} ({code})")
            ...
            if latest.get("signal_sell"):
                if sent_alerts[code]["sell"] != candle_time:
                    ...
                    }.get(latest.get("sell_reason"), "전략 매도")
                    ...
                    if is_held and held_info:
                        ...
                        if order_res and order_res.get("return_code") == 0:
                            sent_alerts[code]["sold_qty"] = qty_to_sell  # Save quantity for rebuy
