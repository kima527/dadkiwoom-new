...
# ─────────────────────────────────────────────────────────
# 최근 매도 발생 시각 기록 (동일 봉 중복 매수 방지 및 쿨타임)
# ─────────────────────────────────────────────────────────
RECENT_SELLS = {}
...
def run_trading_bot():
    ...
    def init_engine_for_code(c):
        ...
        dm = RealtimeDataManager(stock_code=c, max_len=1000, cumulative_limit=limit)
        DATA_MANAGERS[c] = dm
        
        loaded_from_cache = False
        
        if c in global_seed_cache:
            try:
                cd = global_seed_cache[c].get("data", {})
                past_120 = cd.get("120t", [])
                past_1m = cd.get("1m", [])
                past_3m = cd.get("3m", [])
                past_5m = cd.get("5m", [])
                past_15m = cd.get("15m", [])
                past_daily = cd.get("daily", [])
                
                dm.seed_initial_data(past_120, past_1m, past_3m, past_5m, past_15m, past_daily)
                logger.info(f"[{c}] 전날 야간 프리패치된 단일 JSON 캐시에서 시드 데이터 로드 완료 (초고속 O(1))")
                loaded_from_cache = True
            except Exception as e:
                logger.error(f"[{c}] 단일 JSON 시드 데이터 로드 실패: {e}")
                
        if not loaded_from_cache:
            try:
                logger.info(f"[{c}] 로컬 캐시 없음. 비상 시드 데이터 API 다운로드 (1m/3m/5m/15m/daily/120t)...")
                ...
                time.sleep(0.2)
                seed_15m = client.get_15min_candles(c, 30)
                time.sleep(0.2)
