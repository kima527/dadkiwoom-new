...
def calculate_indicators_pure(candles, use_compressed_peak=True, tema_period1=5, tema_period2=20):
    ...
    for i in range(n):
        ...
        if s5 is not None and s20 is not None and s40 is not None:
    ...
    # ── 4-e. SMA Gate Line (SMA 3 & SMA 20) ──
    gate_line_val = None
    for i in range(n):
        # CrossUp: SMA3이 SMA20을 상향돌파하는 순간
        if (i > 0
                and sma3[i] is not None and sma20[i] is not None
                and sma3[i-1] is not None and sma20[i-1] is not None):
            if sma3[i-1] < sma20[i-1] and sma3[i] >= sma20[i]:
                gate_line_val = sma3[i]

        candles[i]['tema_gate_line'] = gate_line_val  # Keep key for backward compatibility

    # 🔒 [CRITICAL LOGIC LOCK - DO NOT MODIFY]
    # ── 5. Signals and Custom State Machine (SMA 3 & SMA 40 Golden/Dead Cross + BB 5/20 Upper Sell & Lower Rebuy) ──
    ...
    for i in range(n):
        ...
        # 1. Buy Signal (SMA 3 > SMA 40 and SMA 40 Slope >= +0.05%)
        is_buy_signal = False
        if (c['sma3'] is not None and c['sma40'] is not None 
            and c_prev is not None and c_prev.get('sma40') is not None and c_prev['sma40'] != 0):
            slope = ((c['sma40'] - c_prev['sma40']) / c_prev['sma40']) * 100
            if c['sma3'] > c['sma40'] and slope >= 0.05:
                is_buy_signal = True

        # 2. Sell Signal Condition 1 (SMA 3 & SMA 40 Dead Cross State)
        is_sell_dead_signal = False
        if c['sma3'] is not None and c['sma40'] is not None:
            if c['sma3'] < c['sma40']:
                is_sell_dead_signal = True

        # Check 15m SMA 5 & SMA 40 Dead Cross State (Macro Exit)
        is_15m_sma_dead = False
        if c['sma5'] is not None and c['sma40'] is not None:
            if c['sma5'] < c['sma40']:
        ...
        if not virtual_holding:
            if waiting_for_bb_rebuy:
                # Check for rebuy cross (Close price Golden Cross SMA5)
                is_rebuy_cross = False
                if (c['close'] is not None and c['sma5'] is not None
                        and c_prev is not None
                        and c_prev.get('close') is not None
                        and c_prev.get('sma5') is not None):
                    if c_prev['close'] < c_prev['sma5'] and c['close'] >= c['sma5']:
                        is_rebuy_cross = True
                ...
                if is_buy_signal and is_buy_window:
                    ...
                    trade_K = c['K_static'] if (c.get('K_static') is not None and c['K_static'] < c['close']) else None
                    c['signal_buy'] = True
                    c['buy_condition_type'] = "SMA 3 > SMA 40"
        ...
        if is_15m_sma_dead:
            c['signal_sell'] = True
            c['sell_reason'] = "15m SMA5-40 Dead Cross"
            virtual_holding = False
            waiting_for_bb_rebuy = False
            monitoring_sell = False
        elif is_bb_sell:
            ...
            c['signal_sell'] = True
            c['sell_reason'] = "SMA 3 Dead Cross"
            virtual_holding = False  # Reset virtual trade state
            waiting_for_bb_rebuy = False
            monitoring_sell = False
        ...
        c['sma5_gt_sma40'] = (c['sma5'] > c['sma40']) if (c['sma5'] is not None and c['sma40'] is not None) else False
        c['sma3_gt_sma40'] = is_buy_signal
        c['signal_sell_sma5_sma40_dead'] = is_15m_sma_dead
        c['signal_sell_sma3_sma40_dead'] = is_sell_dead_signal

    # ── [물리적 방어막] 사용자 지정 신규 로직 (SMA 교체 & Rebuy 기준) 무단 수정 방지 ──
    try:
        if n > 0:
            assert "sma3" in candles[0], "CRITICAL ERROR: SMA 3 지표가 삭제되었습니다."
            assert "sma40" in candles[0], "CRITICAL ERROR: SMA 40 지표가 삭제되었습니다."
            
        import inspect
        code_str = inspect.getsource(calculate_indicators_pure)
        assert "c_prev['close'] < c_prev['sma5'] and c['close'] >= c['sma5']" in code_str, "CRITICAL ERROR: 볼린저밴드 하단 반등 기준(15분봉 종가의 5이평선 상향돌파) 로직이 훼손되었습니다."
        assert "sma3[i-1] < sma20[i-1] and sma3[i] >= sma20[i]" in code_str, "CRITICAL ERROR: 관문선 로직(SMA3이 SMA20 상향돌파)이 훼손되었습니다."
    except Exception as e:
        logger.error(f"방어막 훼손 감지 (indicator.py): {e}")
