...
=============================================================================
15분봉 이평선 수평 응축 + 3·5·20·40·60 정배열 + 수급 폭발 매수 전략 모듈

[전략 조건 및 메커니즘]
1. 15분봉 20, 40, 60 이평선 수평 응축 (Squeeze / Consolidation):
   - 최근 N봉(기본 3봉) 이내 15분봉 20이평, 40이평, 60이평선의 최대/최소 격차가 1.5% 이내로 밀집
   - 장기/중기 매수 매도 주체의 평균 단가가 평탄화되며 변동성 폭발 직전 상태 형성
2. 3·5·20·40·60 완벽한 정배열 전환 (Full Bullish Sequence Alignment):
   - 현재 15분봉 종가 기준 MA3 > MA5 > MA20 > MA40 > MA60 완성
...
class SqueezeAlignmentParams:
    ...
    max_squeeze_pct: float = 1.5          # 20,40,60 이평선 최대/최소 격차 비율 (%) (기본 1.5% 이내 밀집)
    ...
    min_bars_15m: int = 65                # 60이평 계산을 위한 최소 요구 15분봉 데이터 개수


def calculate_15m_squeeze_indicators(df_15m: pd.DataFrame) -> pd.DataFrame:
    """
    15분봉 데이터프레임에 이평선(3, 5, 20, 40, 60), 응축도(Squeeze Ratio), 수급 지표 계산
    """
    df = df_15m.copy()
    close = df['close']
    high = df['high']
    ...
    # 20, 40, 60 이평선 최대/최소 수평 응축 비율 계산
    ma_mid_group = df[['ma20', 'ma40', 'ma60']]
...
def evaluate_15m_squeeze_alignment(
    ...
    '15분봉 20·40·60 이평선 응축 + 3·5·20·40·60 정배열 + 수급 폭발' 조건 충족 여부 판정
    """
    res = {
        "code": code,
        "name": name,
    ...
    # 1. 최근 N봉 이내 20, 40, 60 이평선 응축 조건 검증
    ...
    if not is_squeezed:
        res["reason"] = f"이평선 응축 미충족 (최근 {params.squeeze_lookback_bars}봉 최소 격차: {recent_squeeze_series.min():.2f}% > {params.max_squeeze_pct}%)"
        return res

    # 2. 현재봉 3 > 5 > 20 > 40 > 60 완벽 정배열 검증
    is_aligned = bool(curr['is_full_bullish'])
    res["is_aligned"] = is_aligned
    if not is_aligned:
        res["reason"] = f"3·5·20·40·60 정배열 미충족 (MA3={curr['ma3']:.1f}, MA5={curr['ma5']:.1f}, MA20={curr['ma20']:.1f}, MA40={curr['ma40']:.1f}, MA60={curr['ma60']:.1f})"
        return res
    ...
    res["reason"] = (
        f"🎯 [수급 이평 응축 정배열 돌파 완료] "
        f"응축도={recent_squeeze_series.min():.2f}%, 3-5-20-40-60 정배열, "
