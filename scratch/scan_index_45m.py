...
def run_scan():
    ...
    for idx, s in enumerate(candidates, 1):
        ...
        if idx % 10 == 0 or idx == 1:
            print(f"[{idx}/{total_candidates}] Scanning {name} ({code}) [{market}]...")
            
        candles = get_45min_candles(client, code, last_n_days=10)
        
        # We need at least 60 candles to compute indicators properly (like SMA 60)
        ...
        # Check proximity to L or gate line
        is_close_L = dist_L_pct is not None and abs(dist_L_pct) <= THRESHOLD_PCT
        is_close_gate = dist_gate_pct is not None and abs(dist_gate_pct) <= THRESHOLD_PCT
        
        if is_close_L or is_close_gate:
            # Check trend (sma5 > sma20 > sma60)
            s5 = latest.get("sma5")
            ...
            # Minimum absolute distance to use for sorting
            min_dist = min([abs(x) for x in [dist_L_pct, dist_gate_pct] if x is not None])
    ...
    md_content = f"# 🔍 45분봉 기준선(L/TEMA) 근접 종목 스캔 리포트\n\n"
    md_content += f"- **기준 시간:** {now_str}\n"
    md_content += f"- **대상 지수:** KOSPI 200, KOSDAQ 150 (총 {total_candidates}개 종목)\n"
    md_content += f"- **스캔 기준:** 45분봉 차트 기준, 기준선 L 또는 TEMA 관문선 대비 **±{THRESHOLD_PCT}% 이내** 근접 종목\n"
