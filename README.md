...
### 2. 🔴 매도 로직 (추세 추종 및 상한가 익일 대응)
* **[상한가 익일 시가 대응]**: 당일 상한가(+29% 이상)에 도달한 종목은 홀딩 후, **그 다음 날 당일 시가(Open)를 돌파하지 못하고 하회(현재가 < 시가)** 시 즉시 전량 시장가 매도.
* **[기본 추세 매도]**: **15분봉 단순 5이평선이 40이평선을 하향 돌파(Dead Cross)** 할 때 보유 물량 전량 시장가 매도.
